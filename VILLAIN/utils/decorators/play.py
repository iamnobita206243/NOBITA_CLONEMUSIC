...
from VILLAIN.utils.inline import botplaylist_markup
    ...
    async def wrapper(client, message):
        ...
        if message.sender_chat:
            upl = InlineKeyboardMarkup([
                [InlineKeyboardButton(text="ʜᴏᴡ ᴛᴏ ғɪx ?", callback_data="PROmousAdmin")]
            ])
        ...
        if message.command[0][-1] == "e" and not fplay:
        ...
        if not await is_active_chat(chat_id):
            ...
            try:
                try:
                    get = await app.get_chat_member(chat_id, userbot_me.id)
                ...
                if get.status in [ChatMemberStatus.BANNED, ChatMemberStatus.RESTRICTED]:
                    return await message.reply_text(
                        _["call_2"].format(
                            app.mention, userbot_me.id, userbot_me.first_name, userbot_me.username or "NoUsername"
                        ),
                ...
                try:
                    ...
                    try:
                        await app.approve_chat_join_request(chat_id, userbot_me.id)
                    except Exception as e:
                        return await message.reply_text(_["call_3"].format(app.mention, type(e).__name__))
...
def CPlayWrapper(command):
    async def wrapper(client, message):
        ...
        if not await is_active_chat(chat_id):
            ...
            try:
                try:
                    get = await client.get_chat_member(chat_id, userbot_me.id)
                except ChatAdminRequired:
                    return await message.reply_text(_["call_1"])
                ...
                if get.status in [ChatMemberStatus.BANNED, ChatMemberStatus.RESTRICTED]:
                    return await message.reply_text(
                        _["call_2"].format(
                            i.mention, userbot_me.id, userbot_me.first_name, userbot_me.username or "NoUsername"
                        )
                    )
            except UserNotParticipant:
                if chat_id in clinks:
                    ...
                    if message.chat.username:
                        ...
                        try:
                            ...
                            return await message.reply_text(_["call_3"].format(i.mention, type(e).__name__))
                ...
                try:
                    ...
                    try:
                        await client.approve_chat_join_request(chat_id, userbot_me.id)
                    except Exception as e:
                        return await message.reply_text(_["call_3"].format(i.mention, type(e).__name__))
                    await asyncio.sleep(3)
